...
class PSO:
  def __init__(self, swarmsize: int = 100, omega: int = 0.5, maxiter: int = 100,model: str, device, criterion, X_val, y_val):
  ...
  def fit(self,lb,ub):
    return pso(func= self.objFn, lb, ub, swarmsize=self.swarmsize, omega=self.omega, maxiter=self.max_iter)
